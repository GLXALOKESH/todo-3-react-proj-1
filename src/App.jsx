              ...
              {todos.map((item) => (
                <div className="todo w-full min-h-[40px] flex rounded-full hover:bg-[rgb(52,58,64,0.5)]" key={item.id}>
                  <div className="checkbox w-[35px] h-full flex justify-center items-center relative select-none">
                    ...
                    <img src="/images/tick.svg" alt="" className={item.isCompleted ? 'h-[25px] absolute top-[3px] right-[2px] cursor-pointer' : 'h-[25px] absolute top-[3px] right-[2px] cursor-pointer hidden'} onClick={() => { item.isCompleted = !item.isCompleted; setTodos([...todos]); }} />
                  </div>
                  ...
                  <div className="functions w-[45px] h-full select-none flex justify-center items-center gap-[5px]">
                    <img src="/images/edit.svg" alt="" className='h-[18px] cursor-pointer' onClick={() => { handleEdit(item) }} />
                    <img src="/images/delete.svg" onClick={() => { handleDelete(item.id) }} alt="" className='h-[18px] cursor-pointer' />
                  </div>
                </div>
              ))}
            </div>
